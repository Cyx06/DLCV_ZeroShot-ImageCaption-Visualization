...
class Transformer(nn.Module):
    ...
    def __init__(self, config, d_model=512, nhead=8, num_encoder_layers=6,
                 num_decoder_layers=6, dim_feedforward=2048, dropout=0.1, # change 0.1->0.2
        ...
        # self.encoder = ViTForImageClassification.from_pretrained('google/vit-large-patch16-384')
        # sub_model = nn.Sequential(*list(self.encoder.children())[:-1])
        # self.encoder = sub_model
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, # change 0.1->0.2
                 activation="relu", normalize_before=False):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
